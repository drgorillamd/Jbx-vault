...
// By default, one subscribe to all the strats, equal weights
// Can pass a weight array too
// Pay() => Get share of each strat subscribed (or all if default)

// address[] current strategies -> for the default ones
// address[] deprecated strategies -> for closed strat where still some fund to withdraw
// mapping address project=>address strat=>balance
// mapping strat=>total supply
// mapping address=>strat=>weight

// edge cases:
// shutting down a strat (all the balance should go back to the strategy contract and stay in order to get redeemed)
// a project changing weights (actually ok -> weight is only for new deposits)

contract AppleJuiceTerminalMulti is
    IJBPaymentTerminal,
